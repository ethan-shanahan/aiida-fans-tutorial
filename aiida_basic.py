...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""We can check the contents of the `node` variable like this:""")
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""Now it's time to multiply the two numbers!""")
    return
...
def _(orm_1):
    ...
    _graph = Graph()
    _calc_node = orm_1.load_node(3)
    _graph.add_incoming(_calc_node, annotate_links='both')
    _graph.add_outgoing(_calc_node, annotate_links='both')
    _graph.graphviz
    return (Graph,)
...
@app.cell
def _(orm_1):
    code = orm_1.load_code(label='add')
    code
    return (code,)
...
@app.cell
def _(builder, orm_1):
    builder.x = orm_1.load_node(pk=4)
    builder.y = orm_1.Int(5)
    builder
    return
...
@app.cell
def _(builder, engine_1):
    engine_1.run(builder)
    return
...
def _(Graph, orm_1):
    # Cell tags: hide-input
    _graph = Graph()
    _calc_node = orm_1.load_node(7)
    _graph.recurse_ancestors(_calc_node, annotate_links='both')
    _graph.add_outgoing(_calc_node, annotate_links='both')
    _graph.graphviz
    return
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""You can see more details on any process, including its inputs and outputs, using the verdi shell:""")
    return
...
def _():
    ...
    class MultiplyAddWorkChain(WorkChain):
        ...
        def define(cls, spec):
            ...
            spec.outline(cls.multiply, cls.add, cls.validate_result, cls.result)
            spec.output('result', valid_type=Int)
            spec.exit_code(400, 'ERROR_NEGATIVE_NUMBER', message='The result is a negative number.')

        def multiply(self):
            """Multiply two integers."""
            self.ctx.product = multiply_1(self.inputs.x, self.inputs.y)
...
def _():
    ...
    MultiplyAddWorkChain_1 = _plugins.WorkflowFactory('core.arithmetic.multiply_add')
    return MultiplyAddWorkChain_1, plugins
...
@app.cell
def _(MultiplyAddWorkChain_1, orm_1):
    builder_1 = MultiplyAddWorkChain_1.get_builder()
    builder_1.code = orm_1.load_code(label='add')
    builder_1.x = orm_1.Int(2)
    builder_1.y = orm_1.Int(3)
    builder_1.z = orm_1.Int(5)
    builder_1
    return (builder_1,)
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""Once the `WorkChain` input has been set up, we run it with the AiiDA engine:""")
    return


@app.cell
def _(builder_1, engine_1):
    engine_1.run(builder_1)
    return


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""Now quickly leave the IPython shell and check the process list:""")
    return
...
def _(Graph, orm_1):
    # Cell tags: hide-input
    _graph = Graph()
    _calc_node = orm_1.load_node(14)
    _graph.recurse_ancestors(_calc_node, annotate_links='both')
    _graph.recurse_descendants(_calc_node, annotate_links='both')
    _graph.graphviz
    return
